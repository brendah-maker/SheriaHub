...
@app.route('/')
def health():
    return jsonify({"status": "active", "region": "Kenya"}), 200
...
def ask_ai():
    ...
    try:
        ...
        checkout_id = data.get("checkout_id")  # IMPORTANT: Frontend must send this
        ...
        # 4. THE GATE: Only send deep_dive if paid
        return jsonify({
...
def stk_push():
    try:
        ...
        return jsonify({"error": "M-Pesa push failed"}), 400
